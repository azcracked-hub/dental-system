<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-400">{{ $billings->total() }} total records</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 border-l-4 border-l-yellow-400">
            ...
            <p class="text-2xl font-bold text-gray-900">₱{{ number_format($pendingPayment, 0) }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 border-l-4 border-l-blue-400">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ $billings->total() }}</p>
        </div>
    </div>
            ...
            <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-5">
                <div class="flex items-start justify-between">
                    ...
                    <div class="text-right">
                        ...
                        <span class="text-xs px-2.5 py-1 rounded-full font-medium mt-1 inline-block {{ $bill->status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                            {{ $bill->status }}
                        </span>
                    </div>
                </div>
                ...
                @if($bill->status === 'unpaid')
                    <div class="mt-4 flex gap-3">
                        <button
                            wire:click="openMarkPaidModal({{ $bill->id }})"
    ...
    <x-list-pagination :paginator="$billings" />

    <x-ui.modal :show="$showMarkPaidModal" maxWidth="sm" close-action="closeMarkPaidModal">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-base font-bold text-gray-900">Mark as Paid</h2>
            <button wire:click="closeMarkPaidModal" class="text-gray-400 hover:text-gray-600">✕</button>
        </div>

        <form wire:submit="markPaid" class="space-y-4">
            <div>
                <label class="text-sm font-medium text-gray-700 block mb-1">Payment Method</label>
                <select wire:model="payment_method" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-300">
            ...
            <div class="flex gap-3">
                <button type="button" wire:click="closeMarkPaidModal"
